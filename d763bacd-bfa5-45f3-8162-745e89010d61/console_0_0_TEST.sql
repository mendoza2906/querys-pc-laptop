...
SELECT
    idDocente,idDistributivoDocente,
    nombreDocente,
    idActividadDetalle,
    actividadPersonal,
    dedicacion,
    idDistributivoOferta,
    distributivoOferta,
    idPeriodoAcademico,
    periodoAcademico,
    asignadas,
    ...
    RETURNS TABLE
        ...
        WITH DatosUsuario AS
             ...
             DatosPeriodo AS
                 (
                     SELECT
                         ...
                         pa.fecha_hasta,
                         CASE
                             WHEN @idMes >= MONTH(pa.fecha_desde)
                                 THEN YEAR(pa.fecha_desde)
                             ELSE YEAR(pa.fecha_hasta)
                             END AS anioMes
                     FROM aca.periodo_academico pa
                     WHERE pa.id_periodo_academico = @idPeriodoAcademico
                       AND pa.estado = 'A'
                 ),
             ParametrosFecha AS
                 (
                     SELECT
                         dp.id_periodo_academico,
                         dp.periodoAcademico,
             ...
             Feriados AS
                 (
                     SELECT DISTINCT
                         CAST(ISNULL(f.fecha_traslada, f.fecha) AS DATE) AS fecha
                     FROM pro.feriados f
                     WHERE f.estado = 'A'
                 ),
             RangoPregrado AS
                 (
                     SELECT
                         cs.id_periodo_academico,
                         MIN(cs.fecha_inicio) AS fechaInicio,
                         MAX(cs.fecha_fin) AS fechaFin
                     FROM aca.cal_semana cs
             ...
             RangoNivelacion AS
                 (
                     SELECT
                         ce.id_periodo_academico,
                         MIN(ce.fecha_inicio) AS fechaInicio,
                         MAX(ce.fecha_fin) AS fechaFin
             ...
             ActividadesBase AS
                 (
                     SELECT DISTINCT
                         du.idDocente,
                         du.nombreDocente,
                         acdd.id_actividad_detalle AS idActividadDetalle,
                         apd.descripcion AS actividadPersonal,
                         CONCAT(acdd.codigo, ' ', acdd.descripcion) AS dedicacion,
                         dof.id_distributivo_oferta AS idDistributivoOferta,
                         dof.descripcion AS distributivoOferta,
                         pa.id_periodo_academico AS idPeriodoAcademico,
                         pa.descripcion AS periodoAcademico,
                     ...
                     FROM DatosUsuario du
                              ...
                              INNER JOIN aca.reglamento_actividad_docente rado ON rado.id_reglamento_actividad = rad.id_reglamento_actividad
                         AND rado.id_reglamento = pao.id_reglamento
                              INNER JOIN aca.actividad_personal_docente apd ON apd.id_actividad_personal = rado.id_actividad_personal
                              INNER JOIN aca.fn_distributivo_oferta_max(@idPeriodoAcademico, 'A') dom
                                         ON dom.id_distributivo_oferta = dof.id_distributivo_oferta
                     WHERE pa.id_periodo_academico = @idPeriodoAcademico
                       AND pa.estado = 'A'
                       AND pao.estado = 'A'
             ...
             HorasActividades AS
                 (
                     SELECT
                         ab.idDocente,
                         ab.idActividadDetalle,
                         ab.idDistributivoOferta,
                         CAST(
                                 SUM(
                                         CASE
                                             WHEN fer.fecha IS NULL
                     ...
                     FROM ActividadesBase ab
                              INNER JOIN aca.horario_academico h ON h.id_actividad_detalle = ab.idActividadDetalle
                         AND h.id_periodo_academico = ab.idPeriodoAcademico
                         AND h.id_docente = ab.idDocente
                         AND h.estado = 'A'
                     ...
                     GROUP BY ab.idDocente, ab.idActividadDetalle,
                              ab.idDistributivoOferta
                 ),
             ActividadesResultado AS
                 (
                     SELECT
                         ab.idDocente,ab.idDistributivoDocente,
                         CAST(ab.nombreDocente AS VARCHAR(250)) AS nombreDocente,
                         ab.idActividadDetalle,
                         CAST(ab.actividadPersonal AS VARCHAR(MAX)) AS actividadPersonal,
                         CAST(ab.dedicacion AS VARCHAR(MAX)) AS dedicacion,
                         ab.idDistributivoOferta,
                         CAST(ab.distributivoOferta AS VARCHAR(250)) AS distributivoOferta,
                         ab.idPeriodoAcademico,
                         CAST(ab.periodoAcademico AS VARCHAR(150)) AS periodoAcademico,
                         CAST(ISNULL(ha.horasAsignadas, 0) AS DECIMAL(10, 2)) AS asignadas,
                         ab.idActividadPersonal
                     FROM ActividadesBase ab
                              LEFT JOIN HorasActividades ha ON ha.idDocente = ab.idDocente
                         AND ha.idActividadDetalle = ab.idActividadDetalle
                         AND ha.idDistributivoOferta = ab.idDistributivoOferta
                 ),
             AsignaturasDocente AS
                 (
                     SELECT DISTINCT
                         dd.id_docente AS idDocente,
                         ma.id_malla_asignatura AS idMallaAsignatura,
                         p.id_paralelo AS idParalelo,
                         pa.id_periodo_academico AS idPeriodoAcademico,
                         pa.id_periodo_academico_padre AS idPeriodoAcademicoPadre,dd.id_distributivo_docente as idDistributivoDocente
                     FROM DatosUsuario du
                              INNER JOIN aca.distributivo_docente dd ON dd.id_docente = du.idDocente
                              INNER JOIN aca.distributivo_oferta dof ON dof.id_distributivo_oferta = dd.id_distributivo_oferta
                              INNER JOIN aca.periodo_academico_oferta pao ON pao.id_periodo_academico_oferta = dof.id_periodo_academico_oferta
             ...
             HorariosClases AS
                 (
                     SELECT DISTINCT
                         ...
                         h.hora_inicio,
                         h.hora_fin,
                         ad.idPeriodoAcademico,
                         ad.idPeriodoAcademicoPadre
                     FROM AsignaturasDocente ad
                              INNER JOIN aca.horario_academico h ON h.id_malla_asignatura = ad.idMallaAsignatura
                         AND h.id_paralelo = ad.idParalelo
                         AND h.id_docente = ad.idDocente
                         AND h.estado = 'A'
        ...
        SELECT
            ar.idDocente,ar.idDistributivoDocente,
            ar.nombreDocente,
            ar.idActividadDetalle,
            ar.actividadPersonal,
            ar.dedicacion,
            ar.idDistributivoOferta,
            ar.distributivoOferta,
            ar.idPeriodoAcademico,
            ar.periodoAcademico,
            ar.asignadas,
            ar.idActividadPersonal
        ...
        SELECT
            0 AS idDocente,
            0 as idDistributivoDocente,
            CAST('Clases' AS VARCHAR(250)) AS nombreDocente,
            0 AS idActividadDetalle,
            CAST('Clases' AS VARCHAR(MAX)) AS actividadPersonal,
            CAST('Docencia' AS VARCHAR(MAX)) AS dedicacion,
            0 AS idDistributivoOferta,
            CAST('' AS VARCHAR(250)) AS distributivoOferta,
            @idPeriodoAcademico AS idPeriodoAcademico,
            CAST('' AS VARCHAR(150)) AS periodoAcademico,
            tc.horasClases AS asignadas,
